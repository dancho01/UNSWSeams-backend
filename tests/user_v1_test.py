...
def test_user_stats_base_case():
    ...
    user1_data = user1.json()
    response = requests.get(config.url + 'user/stats/v1',
...
def test_user_stats_users_stats_num_messages_after_removing_messages():
    ...
    stats_response = requests.get(
        config.url + 'users/stats/v1', params={'token': user1_data['token']})
...
def test_user_stats_users_stats_channel_leave():
    ...
    assert type(response_data['user_stats']
                ['channels_joined'][2]['time_stamp']) == int
...
def test_user_stats_users_stats_dm_leave():
    ...
    dm_leave = requests.post(config.url + 'dm/leave/v1',
                             json={'token': user1_data['token'], 'dm_id': dm_data['dm_id']})
...
def test_user_stats_dm_remove():
    ...
    dm_remove = requests.delete(config.url + 'dm/remove/v1',
                                json={'token': user1_data['token'], 'dm_id': dm_data['dm_id']})
...
def test_user_stats_second_user():
    ...
    stats_response = requests.get(
        config.url + 'user/stats/v1', params={'token': user2_data['token']})
    stats_response_data = stats_response.json()
...
def test_user_stats_return_types():
    ...
    user1_data = user1.json()
    stats_response = requests.get(
        config.url + 'user/stats/v1', params={'token': user1_data['token']})
    stats_response_data = stats_response.json()
    assert stats_response.status_code == 200
    assert type(stats_response_data['user_stats']['involvement_rate']) == float
    assert type(stats_response_data['user_stats']
                ['dms_joined'][0]['time_stamp']) == int
    assert type(stats_response_data['user_stats']
                ['messages_sent'][0]['num_messages_sent']) == int
    assert type(stats_response_data['user_stats']
                ['channels_joined'][0]['num_channels_joined']) == int
...
def test_channel_message_invalid_tag(create_first_user, create_second_user):
    ...
    requests.post(config.url + 'message/send/v1', json={'token': user1['token'],
                                                        'channel_id': channel_1_data['channel_id'], 'message': 'z@'})
...
def test_valid_notification_dm_message(create_first_user, create_second_user):
    ...
    requests.post(config.url + 'message/senddm/v1', json={
                  'token': user1['token'], 'dm_id': dm_data['dm_id'], 'message': f'hello@{user2_handle}@hello This is a message'})
...
def test_valid_notification_react(create_first_user, create_second_user):
    ...
    requests.post(config.url + 'message/react/v1', json={
                  'token': user1['token'], 'message_id': message['message_id'], 'react_id': 1})
...
def test_invalid_notification_react_no_longer_in_dm(create_first_user, create_second_user):
    ...
    message_reponse = requests.post(config.url + 'message/senddm/v1', json={
                                    'token': user2['token'], 'dm_id': dm_data['dm_id'], 'message': 'this is message 1'})

    message = message_reponse.json()

    requests.post(config.url + 'dm/leave/v1',
                  json={'token': user2['token'], 'dm_id': dm_data['dm_id']})
