...
def test_dm_remove_not_owner():
    ...
    user1 = requests.post(config.url + 'auth_register/v2', json = {'email' : 'firstuser@gmail.com', 
    'password' : 'password', 'name_first' : 'First', 'name_last' : 'Last'}) 
    user1_data = user.json()
    
    user2 = requests.post(config.url + 'auth_register/v2', json = {'email' :  'seconduser@gmail.com', 
...
def test_dm_remove_user_already_removed():
    requests.delete(config.url + 'clear/v1')
